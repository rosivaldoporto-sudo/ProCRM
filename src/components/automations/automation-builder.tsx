          ...
          <div className="space-y-3 border-t border-border px-4 py-3">
            ...
            {type === "time_based" && (
              <div>
                ...
                <Input
                  placeholder={t("config.placeholderCron")}
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {isCondition ? t("stepTypeCondition") : step.step_type === "wait" ? t("stepTypeWait") : t("stepTypeAction")}
              </div>
...
function previewFor(step: BuilderStep, t: ReturnType<typeof useTranslations>): string {
  switch (step.step_type) {
    ...
    case "send_buttons":
    case "send_list":
      return interactivePayloadPreviewText(asInteractive(step.step_config)) || t("previewNoBody")
    case "send_template":
      return (step.step_config.template_name as string) || t("previewPickTemplate")
    case "wait":
      return `${step.step_config.amount ?? "?"} ${step.step_config.unit ?? ""}`
    case "condition":
      return `${t("previewWhen")} ${step.step_config.subject ?? "?"}`
    case "send_webhook":
