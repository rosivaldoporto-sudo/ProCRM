...
/**
 * Shared Meta Cloud API webhook handler. `accountId` is passed by the
 * per-account route (GET/POST /api/whatsapp/webhook/[accountId]) so
 * every CRM account has its own unique callback URL and the config row
 * is resolved deterministically from the URL. When omitted (legacy URL),
 * the previous verify-token / phone_number_id resolution is used.
 */
